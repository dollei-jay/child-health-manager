...
30. 阶段P2.9（视觉与导航微调）已落地
   - 性别自动换色方案回退，统一保持原女孩配色体系
   - 顶部导航顺序改为：总览 / 生长记录 / 一周计划 / 备忘待办 / 采购清单
   - 顶部、总览、设置与多孩子管理中的头像尺寸整体放大，提升识别度

31. 阶段A1（发布前回归清单）已落地
   - 新增 scripts/release-checklist.md，固化发版前质量门禁
   - 清单覆盖：本地 lint/build/smoke、关键功能回归、移动端视口抽检、发布后抽检与回滚预案

32. 阶段A2.1（切换孩子流畅性）已落地
   - 切换孩子增加“乐观更新 + loading 态”避免卡顿体感
   - 切换按钮在切换中禁用并显示旋转状态，防重复触发
   - 切换失败自动回拉 profile，确保状态一致

33. 阶段A2.2（移动端适配增强）已落地
   - 顶部导航按钮在小屏降密度（字号/内边距收敛）
   - 主内容区移动端边距与垂直间距优化，减少拥挤

34. 阶段A2.3（页面扩展模板化）已落地
   - 新增通用组件 PageSection（标题/副标题/右侧操作区/内容区）
   - 总览页下半区改为模板驱动，后续扩展页面复用成本更低
